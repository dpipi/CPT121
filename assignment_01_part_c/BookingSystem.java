...
public class BookingSystem {
            ...
            switch (choice) {
                case "1": // Ticket Booking
                    ...
                    if (stationDataOK == true) // No escape selected
                    {
                        ...
                        boolean ticketDataOK = ticketMenu(input, selection, firstClass, standardClass, excursionClass);
                        ...
                        if (ticketDataOK == true)// No escape selected
                        {
                            ...
                            receiptNumber = ticketBooking(selection, firstClassSeats, standardClassSeats,
                                    excursionClassSeats, receiptNumber, firstClass, standardClass, excursionClass,
    ...
    public static boolean ticketMenu(Scanner input, Object[] selection, TrainService firstClass,
            TrainService standardClass, TrainService excursionClass) {
                    ...
                    while (adultInputOK == false) // while information incorrect
                    {
                        ...
                        for (int row = (int) selection[0]; row < (int) selection[1]; row++) {
                            ...
                            } else // All details not ok
                            {
                                ...
                                System.out.println("ERROR - TOTAL GREATER THAN AVAILABLE SEATS");
                                System.out.println("ENTER NEW AMOUNT OR X TO RETURN TO MENU\n");
                                break;
                            }
                        }
                    ...
                    if (abort == false) {
                        ...
                        while (childPaidInputOK == false) {
                            ...
                            for (int row = (int) selection[0]; row < (int) selection[1]; row++) {
                                ...
                                if (childPaidTickets + adultTickets <= standardClass.checkSeatAmount(row)) {
                                    okToBook = true;// All details ok to book tickets
                                } else {
                                    // display ticket availability menu
                                    Availability(firstClass, standardClass, excursionClass);
                ...
                case "3": // EXCURSION CLASS
                    ...
                    if (abort == false) {
                        ...
                        while (childPaidInputOK == false) {
                            ...
                            for (int row = (int) selection[0]; row < (int) selection[1]; row++) {
                                ...
                                if (childPaidTickets + adultTickets <= excursionClass.checkSeatAmount(row)) {
                                    okToBook = true;// All details ok to book tickets
                                } else {
                                    // display ticket availability menu
                                    Availability(firstClass, standardClass, excursionClass);
                    ...
                    if (abort == false) {
                        ...
                        while (childInputOK == false) {
                            ...
                            for (int row = (int) selection[0]; row < (int) selection[1]; row++) {
                                // test seats accross all stops for class
                                if (childFreeTickets + childPaidTickets + adultTickets <= excursionClass
                                        .checkSeatAmount(row)) {
                                    okToBook = true;// All details ok to book tickets
                                } else {
                                    // display ticket availability menu
    ...
    public static int ticketBooking(Object[] selection, Object[][] firstClass, Object[][] standardClass,
            Object[][] excursionClass, int receiptNumber, TrainService firstClassSeats, TrainService standardClassSeats,
            TrainService excursionClassSeats, BookingReceipt<?> receipt, boolean displayForTeacher, Scanner input) {
        ...
        if (selection[2] == "First Class") {
            ...
            for (int bookings = 0; bookings < ((int) selection[3]); bookings++) {
                ...
                for (int checkTickets = 0; checkTickets < ((int) selection[3]); checkTickets++) {
                    for (seatNumber = 0; seatNumber < 48; seatNumber++) {

                        seatClear = false; // initialize variable state

                        // test each seat from selected boarding location to departing location
                        innerBreak: for (int trainStop = ((int) selection[0]); trainStop < ((int) selection[1]); trainStop++) {
                        ...
                        if (seatClear == true) { // seat positions free
                            break;
                        }
                    }
                }

                if (seatClear == true) { // Seat is available

                    if (receiptCounter == 0) { // generate first receipt

                        newReceipt = receipt.createReceipt(); // Booking receipt method( creates string receipt
                    ...
                    for (int bookTickets = 0; bookTickets < ((int) selection[3]); bookTickets++) {
            ...
            // do for ticket amount total
            for (int bookings = 0; bookings < (((int) selection[3] + (int) selection[4]
                ...
                if (seatClear == true) { // if seat not taken
            ...
            for (int bookings = 0; bookings < (((int) selection[3] + (int) selection[4]
                ...
                for (int checkTickets = 0; checkTickets < ((int) selection[3]); checkTickets++) {
                    for (seatNumber = 0; seatNumber < 95; seatNumber++) {
                        ...
                        // test each seat from selected boarding location to departing location
                        innerBreak: for (int j = ((int) selection[0]); j < ((int) selection[1]); j++) {
                ...
                if (seatClear == true) { // if seat not taken
                    ...
                    for (int bookTickets = 0; bookTickets < ((int) selection[3]); bookTickets++) {
                        for (int k = ((int) selection[0]); k < ((int) selection[1]); k++) {
                            // sets receipt number into array seat location
                            excursionClassSeats.setSeat(k, seatNumber, (newStandardReceipt));
                        }
                    }
                }
            }
    ...
    public static void Availability(TrainService firstClass, TrainService standardClass, TrainService excursionClass)
    //
...

    {
        ...
        System.out.printf("     %s     |   %s   |   %s    |    %s    |\n", "Seat Availability", "Class", "Class",
                "Class");
        System.out.println("-".repeat(67));
        System.out.printf("Castlemaine ->   Campbell  |    %d     |     %d     |      %d     |\n",
                firstClass.checkSeatAmount(0), standardClass.checkSeatAmount(0), excursionClass.checkSeatAmount(0));
        System.out.println("-".repeat(67));
        System.out.printf("Campbell    ->   Guildford |    %d     |     %d     |      %d     |\n",
                firstClass.checkSeatAmount(1), standardClass.checkSeatAmount(1), excursionClass.checkSeatAmount(1));
        System.out.println("-".repeat(67));
        System.out.printf("Guildford   ->   Strangway |    %d     |     %d     |      %d     |\n",
                firstClass.checkSeatAmount(2), standardClass.checkSeatAmount(2), excursionClass.checkSeatAmount(2));
        System.out.println("-".repeat(67));
        System.out.printf("Strangway   ->   Newstead  |    %d     |     %d     |      %d     |\n",
                firstClass.checkSeatAmount(3), standardClass.checkSeatAmount(3), excursionClass.checkSeatAmount(3));
        System.out.println("-".repeat(67));
        System.out.printf("Newstead    ->   Moolort   |    %d     |     %d     |      %d     |\n",
                firstClass.checkSeatAmount(4), standardClass.checkSeatAmount(4), excursionClass.checkSeatAmount(4));
        System.out.println("-".repeat(67));
        System.out.println("\n\n");
